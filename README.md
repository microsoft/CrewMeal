...
복잡한 사내 문서(`.pptx`·`.pdf`·`.hwp`·`.hwpx`)를 구조화된 검색 메타데이터로 강화해 Microsoft Graph
...
payload가 없는 HWP 페이지만 선택적으로 렌더링·분석합니다. 결과는 허용 태그 HTML로
렌더링하며 원본 문서는 수정하지 않습니다. 분석 모델은 관리 포털에서 교체할 수 있고,
