...
- Microsoft Foundry `AIServices` S0 계정 + `gpt-5.2` / `gpt-5-mini` / `text-embedding-3-large` 배포
...
- `GET /admin` — 관리자 포탈(대시보드). `X-Admin-Key` 헤더 또는 로그인 세션으로 게이트하며
  ...
  - `/admin/tryout` — PPTX를 직접 업로드해 파이프라인을 시험하는 플레이그라운드
...
전용 테스트 사이트에서 선택한 PPTX만 추가 `externalItem`으로 색인하는 opt-in
워크플로입니다. 원본 PPT는 수정하지 않고 SharePoint 기본 검색에도 그대로 남겨
...
워커는 PPTX를 임시 폴더에만 다운로드하고, 144 DPI 이미지와 보이는 Open XML을
GPT-5-mini에 전달합니다. 모델의 strict JSON Schema 응답은 코드가 허용 태그
HTML로 렌더링합니다. 발표자 노트는 별도 섹션으로 추가하며 원본 PPTX, PDF, PNG,
HTML 본문과 비밀 값은 SQLite에 저장하지 않습니다.
